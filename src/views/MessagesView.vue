      ...
      <section :class="['qq-shell', { 'qq-shell--chat-open': mobilePane === 'chat' }]">
        <aside class="qq-sidebar">
          ...
          <div class="qq-sidebar__summary">
            ...
            ><strong>{{ flatThreads.length }}</strong>
          </div>
          <div class="qq-sidebar__groups">
            ...
            <p v-else-if="showSearchEmpty" class="qq-search-empty">
        ...
        <section class="qq-chat">
          ...
          <div v-if="selectedThread" ref="chatHistoryRef" class="qq-chat__history">
            ...
            <template v-for="group in groupedMessages" :key="group.key">
              ...
              <template v-for="(message, idx) in group.messages" :key="message.id">
                <div v-if="shouldShowUnreadDivider(message.id)" class="qq-chat__unread-divider">
                ...
                <div :class="messageRowClass(message, idx, group.messages)">
                  <div
                    ...
                    :class="messageAvatarClass(message, idx, group.messages)"
                  >
                  ...
                  <div
                    ...
                    :class="[
                      ...
                      {
                        'qq-message-avatar--hidden': isSameSenderAsNext(
                          message,
                          idx,
                          group.messages,
                        ),
                  ...
                  <div class="qq-message-group">
                    <div
                      v-if="shouldShowSenderInfo(message, idx, group.messages)"
          ...
          <div v-else class="qq-chat__empty">
            <div class="qq-chat__empty-card">
              <div class="qq-chat__empty-avatar">{{ chatTarget ? chatTarget.avatar : '...' }}</div>
              <h3>
                {{
                  chatTarget ? uiStore.t('messagesGreetTitle') : uiStore.t('messagesChooseTitle')
                }}
              </h3>
              <p>
                {{ chatTarget ? uiStore.t('messagesGreetText') : uiStore.t('messagesChooseText') }}
              </p>
            </div>
          </div>
          <div v-if="chatTarget" class="qq-composer">
            <div class="qq-composer__field">
              ...
              <div class="qq-composer__meta">
                ...
                ><small
                  :class="[
                    'qq-composer__count',
                    { 'qq-composer__count--warning': draftRemaining <= 60 },
                  ]"
                  >{{ draftLength }}/{{ MESSAGE_CHARACTER_LIMIT }}</small
                >
              </div>
            ...
            <button
              type="button"
              :disabled="messagesStore.sending || !draft.trim()"
              @click="handleSend"
            >
...
const routeTarget = computed(() => {
  if (
    ...
    !route.query.name ||
    !route.query.role ||
    route.query.userId === authStore.currentUser?.id
  )
    return null
  return {
...
const flatThreads = computed(() =>
  normalizedThreadSearch.value
    ? threads.value.filter((thread) => matchesThreadSearch(thread))
    : threads.value,
)
...
const showSearchEmpty = computed(
  () =>
    Boolean(normalizedThreadSearch.value) && !flatThreads.value.length && threads.value.length > 0,
)
...
const sendingText = computed(() =>
  uiStore.locale === 'zh' ? '\u6b63\u5728\u53d1\u9001...' : 'Sending...',
)
const mobileThreadLabel = computed(() => (uiStore.locale === 'zh' ? '\u4f1a\u8bdd' : 'Chats'))
const allChatsTitle = computed(() =>
  uiStore.locale === 'zh' ? '\u5168\u90e8\u4f1a\u8bdd' : 'All chats',
)
...
function buildThreadId(userA, userB) {
...
function shouldShowSenderInfo(message, index, messages) {
  return (
    message.senderId !== authStore.currentUser?.id &&
    (index === 0 || messages[index - 1]?.senderId !== message.senderId)
  )
}
function isSameSenderAsPrev(message, index, messages) {
  return index > 0 && messages[index - 1]?.senderId === message.senderId
}
function isSameSenderAsNext(message, index, messages) {
...
function messageAvatarClass(message, index, messages) {
  return [
    'qq-message-avatar',
    { 'qq-message-avatar--hidden': isSameSenderAsNext(message, index, messages) },
  ]
}
...
function formatMessageTime(value) {
  return new Date(value).toLocaleString(uiStore.locale === 'zh' ? 'zh-CN' : 'en-US', {
    month: 'short',
    day: 'numeric',
    hour: 'numeric',
    minute: '2-digit',
...
function formatClockTime(value) {
  return new Date(value).toLocaleTimeString(uiStore.locale === 'zh' ? 'zh-CN' : 'en-US', {
    hour: '2-digit',
    minute: '2-digit',
  })
}
function buildThreadPreview(thread) {
  return thread.latestMessage.senderId === authStore.currentUser?.id
    ? `${uiStore.locale === 'zh' ? '\u6211: ' : 'You: '}${thread.latestMessage.content}`
    : thread.latestMessage.content
}
function matchesThreadSearch(thread) {
  return [
    ...
    .join(' ')
    .toLowerCase()
    .includes(normalizedThreadSearch.value)
}
...
async function scrollChatToBottom() {
  await nextTick()
  if (selectedThread.value && chatHistoryRef.value)
...
function shouldShowUnreadDivider(messageId) {
  return Boolean(openedUnreadMessageId.value) && openedUnreadMessageId.value === messageId
}
...
async function syncMessageSession(currentUserId) {
  if (!currentUserId) {
...
watch(
  ...
  (currentUserId) => {
    void syncMessageSession(currentUserId)
  },
  { immediate: true },
)
...
watch(
  routeTarget,
  (nextRouteTarget) => {
    if (nextRouteTarget) mobilePane.value = 'chat'
    else if (!selectedThread.value) mobilePane.value = 'threads'
  },
  { immediate: true },
)
watch(
  ...
  ([currentUserId, threadId, latest]) => {
    if (currentUserId && threadId) {
      messagesStore.setLastActiveThread(currentUserId, threadId)
      messagesStore.markThreadRead(currentUserId, threadId, latest)
    }
  },
  { immediate: true },
)
...
watch([currentDraftKey, draft], ([nextDraftKey, nextDraft]) => {
  ...
  if (nextDraft.trim()) nextStored[nextDraftKey] = nextDraft
  else delete nextStored[nextDraftKey]
  storedDrafts.value = nextStored
  writeStoredDrafts(nextStored)
})
watch(
...
async function handleSend() {
  ...
  if (currentDraftKey.value) {
    const nextStored = { ...storedDrafts.value }
    delete nextStored[currentDraftKey.value]
    storedDrafts.value = nextStored
    writeStoredDrafts(nextStored)
  }
  draft.value = ''
...
.qq-sidebar {
  ...
  background: linear-gradient(180deg, rgba(248, 250, 255, 0.72), rgba(255, 255, 255, 0.48));
}
...
.qq-chat {
  ...
  min-width: 0;
  background:
    linear-gradient(180deg, rgba(255, 255, 255, 0.64), rgba(248, 250, 255, 0.5)),
...
.qq-composer button {
  ...
  border: none;
  border-radius: var(--radius-pill);
  background: linear-gradient(135deg, #1890ff, #60a5fa);
  color: #fff;
  font-weight: 700;
